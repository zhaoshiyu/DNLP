...
num_steps = 20 # it must consist with the test

start_time = time.time()
print("preparing train and validation data")
X_train, y_train, X_val, y_val = data_loader.getTrain(train_path=train_path, val_path=val_path, seq_max_len=num_steps)
...
if emb_path != None:
    embedding_matrix = data_loader.getEmbedding(emb_path)
else:
